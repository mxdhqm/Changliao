...
@interface UApplyViewController () <ApplyFriendCellDelegate>

@end

@implementation UApplyViewController

static UApplyViewController *controller = nil;
...
- (void)addNewApply:(NSDictionary *)dictionary
{
    ...
    if (dictionary && [dictionary count] > 0) {
        ...
        if (applyUsername && applyUsername.length > 0) {
            for (i = ((int)[_dataSource count] - 1); i >= 0; i --) {
                ApplyEntity *oldEntity = [_dataSource objectAtIndex:i];
                ApplyStyle oldStyle = [oldEntity.style intValue];
                if ([applyUsername isEqualToString:oldEntity.applicantUsername]) {
